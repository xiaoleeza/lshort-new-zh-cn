...
\texttt{latex} 命令及 \hologo{pdfLaTeX} 命令下，这些编码的支持由 \pkg{inputenc} 宏包提供。比如将文档保存为 Latin-1 编码，并在导言区使用：
\begin{verbatim}
...
\texttt{latex} 命令及 \hologo{pdfLaTeX} 命令下可以使用 \pkg{inputenc} 宏包支持 UTF-8：
\begin{verbatim}
\usepackage[utf8]{inputenc}
\end{verbatim}

\hologo{XeLaTeX} 原生支持 UTF-8 编码，而且也\textbf{不适用 \pkg{inputenc} 宏包}。将源代码以 UTF-8 格式保存，
...
\footnote{现在的 \hologo{pdfLaTeX} 以及 \texttt{latex} + \texttt{dvipdfmx} 通过字体映射的方式支持直接使用中文字体，不需要经过预处理，
但仍然是不方便新手操作的。}。
...
用 \texttt{latex}+\texttt{dvipdfmx} 命令、\hologo{pdfLaTeX} 或 \hologo{XeLaTeX} 命令（不支持 GBK）都能够编译。
推荐使用 UTF-8 编码编写源代码，用 \hologo{XeLaTeX} 命令编译。
