...
\begin{enumerate}
  ...
  如果想在数学公式中输入正体的文本，简单情况下用 \ref{subsec:math-alpha} 中提供的 \cmd{mathrm} 命令，或者 \pkg{amsmath} 提供的更方便的 \amscmd{text} 命令%
...
\env{split} 环境和 \env{aligned} 环境用法类似，也用于和 \env{equation} 环境套用，区别是 \env{split} 只能将每行的一个公式分两栏，
\env{aligned} 能分多栏。
...
在 \cmd{align} 中使用则会令 \qedsymbol\ 盖掉公式的编号；使用 \env{equation} 嵌套 \amsenv{aligned} 环境时，
