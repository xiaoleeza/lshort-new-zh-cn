...
\LaTeX\ 原生的数学公式排版的扩展，其核心是 \pai{amsmath} 宏包，对多行公式的排版提供了有力的支持。
此外，\pai{amsfonts} 宏包以及基于它的 \pai{amssymb} 宏包提供了丰富的数学符号；\pai{amsthm} 宏包扩展了 \LaTeX\ 定理证明格式。
...
\subsection{数学模式浅谈}\label{subsec:math-mode}
...
\begin{enumerate}
  ...
  如果想在数学公式中输入正常字体的文本，\pkg{amsmath} 提供了一个方便的 \amscmd{text} 命令%
...
\pkg{amssymb} 宏包提供了一些次常用的符号。这些符号全部能在本章末尾的 \ref{sec:math-tables} 节列出的表格里查到。
...
大于等于号 $\ge$ （\cmd{ge}）、小于等于号 $\le$ （\cmd{le}）、
约等号 $\approx$ （\cmd{approx}）、等价 $\equiv$ （\cmd{equiv}）、相似 $\sim$ （\cmd{sim}）%
\footnote{经常用来替代 \cmd{\textasciitilde\{\}} 作为波浪号。但注意\textbf{只能在数学模式使用}。}等等。
更多符号命令可参考表 \ref{tbl:math-rel} 以及表 \ref{tbl:ams-rel}。

\LaTeX\ 还提供了自定义二元关系符的命令 \cmd{stackrel}\marg*{...}\marg*{...}，用于将一个符号叠加在原有的二元关系符之上：
\begin{example}
\[
...
如乘号 \msym{times}、 除号 \msym{div}、点乘 \msym{cdot}、加减号 \msym{pm} 等等。
...
\hline
\ci{sin} & \ci{arcsin} & \ci{sinh} & \ci{exp} & \ci{dim} \\
\ci{cos} & \ci{arccos} & \ci{cosh} & \ci{log} & \ci{ker} \\
\ci{tan} & \ci{arctan} & \ci{tanh} & \ci{lg}  & \ci{hom} \\
\ci{cot} & \ci{arg}    & \ci{coth} & \ci{ln}  & \ci{deg} \\
\ci{sec} & \ci{csc}    & \\
\hline
\multicolumn{5}{c}{带上下限的算符} \\
\hline
\ci{lim} & \ci{limsup} & \ci{liminf} & \ci{sup} & \ci{inf} \\
\ci{min} & \ci{max}    & \ci{det}    & \ci{Pr}  & \ci{gcd} \\
\hline
\end{tabular}
\end{table}
...
上表中的算符如果不满足使用的话，\pkg{amsmath} 允许用户用 \amscmd{Declare\-Math\-Operator} 
...
\pkg{amsmath} 宏包还提供了 \amscmd{substack} 命令，能够在上下限位置书写多行表达式：
\begin{example}
\[
...
、表示向量的箭头 $\vec{r}$ (\cmd{vec}\marg*{r}) 等，详见表 \ref{tbl:math-accents}。
...
读者可能阅读过其它手册或者资料，知道 \LaTeX\ 有个 \env{eqnarray} 环境。它按照等号左边--等号--等号右边呈三列对齐，
但等号周围的空隙过大，加上公式编号等一些 bug，目前已不推荐使用。

目前最常用的是 \amsenv{align} 环境，它将公式用 \texttt\& 隔为两部分并对齐。分隔符通常放在等号左边（放在右边会使符号间距不太正常）：
\begin{example}
\begin{align}
a & = b + c \\
& = d + e
...
\section{公式中的空格}\label{sec:math-space}
...
数学符号按照符号出现的位置规定尺寸，从大到小包括行间公式尺寸、行内公式尺寸、上下标尺寸、次级上下标尺寸。
...
\pkg{amsmath} 包含的子宏包 \pkg{amsbsy} 提供了一个 \amscmd{boldsymbol} 命令，用于打破 \cmd{boldmath} 的限制，
在公式内部将一部分符号切换为粗体。
\begin{example}
$\mu, M \qquad
...
\LaTeX\ 工具宏集之一的 \pkg{bm} 宏包可以用 \cmd{bm} 生成“伪粗体”，一定程度上解决了不带粗体版本的符号的问题。这里不做过多介绍，
...
其中 \Arg{type} 为定理类型的名称，作为一个环境来使用。\Arg{title} 是定理类型的标签（“定理”，“公理”等），排版在序号之前。
...
例如，我们用以下代码定义了一个 \env{mytheorem} 环境：
\begin{verbatim}
\newtheorem{mytheorem}{My Theorem}[chapter]
\end{verbatim}
\newtheorem{mytheorem}{My Theorem}[chapter]

于是我们可以使用 \env{mytheorem} 环境排版定理。定理带一个可选参数，用于注明定理的名称，如“法拉第定律”等。
...
\texttt{des\-crip\-tion} 使用粗体标签、正体内容；\texttt{remark} 使用斜体标签、正体内容。
...
在 \cmd{align} 中使用则会盖掉公式的编号；使用 \env{equation} 嵌套 \amsenv{aligned} 环境时，
\cmd{qedhere} 会将 \qedsymbol 直接放在公式后。这些位置都不太正常。
