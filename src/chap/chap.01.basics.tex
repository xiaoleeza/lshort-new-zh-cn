...
\end{tabular}
\end{center}
\footnotetext{现今的发行版中 \texttt{latex} 命令也用 \hologo{pdfTeX} 引擎结合 \LaTeX\ 格式，生成 DVI 格式的文档。}
...
\item[\texttt{titlepage}, \texttt{notitlepage}] 设定标题命令 \cmd{maketitle} 是否单独成页。
...
\item[\texttt{openright, openany}] \quad 设定新的一章 \cmd{chapter} 是在奇数页（右侧）开头，还是直接紧跟着上一页开头。
...
\cmd{usepackage}的参数里可以使用不止一个宏包，多个宏包用逗号隔开。这种用法一般不要加选项%
