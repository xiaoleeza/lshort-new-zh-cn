...
\begin{itemize}
  ...
  \TeX\ Live 自 2011 年起以年份作为发行版的版本号，保持了一年一更的速度。

  Mac\TeX\ 是 OS X 系统下的一个定制化的 \TeX\ Live 版本，与 \TeX\ Live 同步更新。

  \item \textbf{Mik\TeX}\par
  Mik\TeX\ 是主要用于 Windows 平台的一个稳定发展的 \TeX\ 发行版，对 Linux 平台的支持不如 \TeX\ Live。
...
可用宏包管理器安装。如果你知道某个宏包的名称，但不确定是否在发行版中已打包，
可浏览 \nolinkurl{http://www.ctan.org/pkg/}\Arg{package-name} 查询。
